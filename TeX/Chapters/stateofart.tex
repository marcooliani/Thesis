...
\bigskip
What severely limits the analysis performed with the tool implemented by Ceccato et al. is the use of \textit{ad hoc} solutions and \textit{a posteriori} interventions done manually on the datasets after the collection process: I will discuss this last aspect in more detail later.\newline
...
\paragraph{Testbed}
The testbed environment used by Ceccato et. al is entirely simulated, from the physical system to the control system. The PLCs were built with \textbf{OpenPLC} \cite{openplc} in the Docker environment, while the physics part was built through Simulink.
...
\bigskip
The biggest problem with the testbed, however, is not with the controller part, but with the physical part
