...
\label{sub:enip}
\textit{Ethernet/IP} (where IP stands for \textit{Industrial Protocol}) is an industrial protocol that allows the \textit{Common Industrial Protocol} (CIP) to run on a typical Ethernet network \cite{enip_pdf}.

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.40]{enip.png}
	\caption{Ethernet/IP stack compared to the OSI stack}
	\label{fig:enip}
\end{figure}

Ethernet/IP uses the main Ethernet standards, such as IEEE 802.3 and the TCP/IP suite, and implements the CIP protocol stack at the upper layers of the OSI stack (see Figure \ref{fig:enip}). It is furthermore compatible with the main Internet standard protocols, such as SNMP, HTTP, FTP and DHCP, and other industrial protocols for data access and exchange such as \textit{Open Platform Communication} (OPC).
